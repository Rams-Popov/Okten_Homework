...
const createDiv = (objectArray, divka) =>{
    for (const element in objectArray) {
        if (typeof objectArray[element] === 'object'){
            const div = document.createElement('div');
            createDiv(objectArray[element], div);
            divka.appendChild(div);
        } else {
            const div = document.createElement('div');
            div.innerHTML = objectArray[element];
            divka.appendChild(div);
        }
    }
}
...
const findAllClasses = (where) =>{
    ...
    for (const el of element) {
        ...
        if(el.tagName === "P") {
            htmlObject.paragraphs.push(el.textContent);
        }
        if(el.tagName === "H1" || el.tagName === "H2") {
            htmlObject.headings.push(el.textContent);
        }
    }
}
